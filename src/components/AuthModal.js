...
export default function AuthModal({ visible, onClose, signInWithEmail, signUpWithEmail, signInWithGoogle, onContinueGuest }) {
  ...
  const submit = async () => {
    ...
    if (mode === 'signUp' && !displayName.trim()) return setMessage('請輸入你的舞者名稱。');
    if (mode === 'signUp' && password !== confirmPassword) return setMessage('兩次輸入的密碼不一致。');
    setBusy(true); setMessage('');
    const result = await (mode === 'signIn' ? signInWithEmail?.(email.trim(), password) : signUpWithEmail?.(email.trim(), password, displayName));
    setBusy(false);
