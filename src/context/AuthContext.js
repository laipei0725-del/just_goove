...
export function AuthProvider({ children }) {
  ...
  const value = useMemo(() => ({
    ...
    signUpWithEmail: (email, password, displayName) => supabase ? supabase.auth.signUp({ email, password, options: { data: { display_name: displayName?.trim() || undefined } } }) : Promise.resolve(unavailable),
